...
Fingerprint is a technique only to identify a copy of a file by generating a cryptographic hash. With Watermark, these techniques can be used for a posteriori protection. Because identification does not control the permission, the user can do whatever s/he wants with the file. The idea is, if illegal content appears for example on a peer-to-peer network, to be able to find the user who did it.
...
The last task produced for the Next Media project was the participation in the reflection group about e-book protection initiated by the Finnish Book Publishers Association. The outcome of this work is the DRM recommendation sent to the Finnish publishers in June 2013. The main result of this publication is that two major Finnish publishers abandoned DRM in favour of the watermark and Social DRM lighter protection system\footnote{\url{https://www.sttinfo.fi/release?releaseId=8700912}}. The DRM recommendation was accompanied by the Web crawler text presented in appendix \ref{app:crawl} or as a stand alone text in github\footnote{\url{https://github.com/patrick-ausderau/thesis}}
...
The Social DRM and watermark combination might be the least problematic technical solution since it does not impose restriction on usage, but offers some level of protection and helps to deter from copyright infringement. However, it raises privacy issues and could contribute in restricting the freedom of opinion.
