...
In the normal flow (see figure \vref{fig:def:adobe_cs}), the user visits the publisher or retailer web store to buy an e-book. After the purchase confirmation, he will receive an Adobe Content Server Message (.acsm) file and by opening it with the Adobe Digital Editions, the software will communicate online with the Content Server that will encrypt the e-book and provides it DRM protected with the publisherâ€™s authorizations for the user based on his adobe ID. Before the user can read the e-book, he must also have registered his device. That way, the platform makes sure that the file owned by the user is not distributed further. 
\begin{figure}[h]
...
This work research if a DRM system can be made to technically protect an e-book while at the same time reduce the restriction to the fair use. Currently, one drawback of the DRM system is the lack of interoperability, like the impossibility to read an e-book on different platform. Thus, it become interesting to test if the upcoming W3C Web Cryptography API\footnote{\url{http://www.w3.org/TR/WebCryptoAPI}} standard could be used for the protection and encryption part of such DRM system.
...
Another participation to the Next Media project consisted of a presentation of some results of this work to the Finnish National library and the company Ellibs representatives in April 2012. The presentation and discussion concentrated on the topics of how could Finnish libraries lend e-books the same way they do with paper books and also about the archiving e-books. An other presentation for the Finnish Book Publishers Association development group in October 2012 concentrated on presenting the different alternatives for protecting e-books.

For the eReading work-package of the Next Media project, there were two presentations in seminars, one in June 2012 and the next one in December 2012. Another seminar participation was in the Next Media Result Seminar in February 2013, with a short, fifteen minutes presentation. The slide show of this presentation is available on the Next Media website\footnote{\url{http://www.nextmedia.fi/779350088/en/read/Next_Media_Result_Seminar_6.2.2013}} and the video recording is available on the Vimeo platform\footnote{\url{http://vimeo.com/59498270}}. A last presentation is a 45 minutes conference for the Finnish Book Publishers Association and two authors unions members in February 2012. The slide show of the presentations are published on github\footnote{\url{https://github.com/patrick-ausderau/thesis}}.

The last task produced for the Next Media project is the participation in the reflection group about e-book protection initiated by the Finnish Book Publishers Association. The outcome of this work is the DRM recommendation send to the Finnish publishers in June 2013. The main result of this publication is that two majors Finnish publishers abandoned DRM in favour of the watermark and Social DRM lighter protection system\footnote{\url{https://www.sttinfo.fi/release?releaseId=8700912}}. The DRM recommendation was accompanied by the Web crawler text presented in the annex \ref{app:crawl} or as a stand alone text in github\footnote{\url{https://github.com/patrick-ausderau/thesis}}

The other part of the products of this thesis is an application work demonstrating that the upcoming W3C Web Cryptography API can be used to encrypt data. It also shows some of the limitation of DRM systems such as the difficulty to hide private keys. It also outline the risks in using the current implementation of the Web Cryptography API for serious cryptographic operations.


\chapter{Discussion}\label{discuss}

During this research work, I studied the existing and upcoming technical protection on e-book, namely: Digital Right Management (DRM), Watermark, Social DRM and Lightweight Content Protection (LCP) and tried to implement a protection system with the Web Cryptography API. What I found is that it does not exist possible improvement of the DRM techniques because of the paradigm problem of trying to impose the physical world to the electronic world which are fundamentally different. With a major difference being the ease of copying and sharing world wide.
...
At the non technical level, there is also important lewis that can be used to fight copyright infringements. At the market level, it is important that the publishers offer a rich collection of titles in electronic form, makes it easily available, propose attractive prices and ideally do not impose DRM restriction\cite[p. 13]{OpinionWay:baro_v2}. With that approach, the user would be less tempted to search for the illegal alternatives. Another approach that would be interesting to study could be to test Crowdfunding where the author writes and publishes his new book only if he gets its funding beforehand.
...
Under the current copyright laws, the author already benefit from copyright protection for his e-book even if he publishes it without technical protection. He could also surrender some of his rights, such as allowing adaptation or sharing, using one of the Creative Commons\footnote{\url{http://creativecommons.org}},  Copyheart\footnote{\url{http://copyheart.org}}, Copyfarleft\footnote{\url{http://p2pfoundation.net/Copyfarleft}} or Free Art\footnote{\url{http://artlibre.org/licence/lal/en}} licenses.

This research work was limited to the copyright protection on electronic book. However, the question of copyright protection also applies on other artistic and scientific domains and this work open the way to new reflections in theses domains as many of the proposition made could easily be extended to the arts and sciences in general. Indeed, this problem and the proposed solution already apply to the music and cinema and with the advance of technology such as 3D printer, this could also reach other domains such as sculpture and carving. There is research on holographic screen, so maybe one day, we will be able to have live arts such as dancing and theatre in our living room as well, making the adaptation of the copyright a still existing question in the future.

\chapter{Conclusions}\label{conc}

The  need to have copyright for e-book being respected, such as the moral and material interests of the author is legitimate. But the research conducted shows that among the technical options to protect e-book from illegal uses, none is perfect. Trying to impose the rules of the physical world to the electronic one is not suitable. 
